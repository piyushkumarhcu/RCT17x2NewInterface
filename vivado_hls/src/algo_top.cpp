...
void processInputData(ap_uint<384> &link, CrystalGroup &crystalGroup) {
#pragma HLS PIPELINE II=6
#pragma HLS INLINE
  crystalGroup = CrystalGroup(link);
  return;
}
...
#pragma HLS UNROLL
    size_t end = start + 31;
    link_out[0].range(end, start) = stitchedTowerLevelECALSummary[tower];
    start += 32;
  }
...
#pragma HLS UNROLL
    size_t end = start + 31;
    link_out[1].range(end, start) = stitchedTowerLevelECALSummary[tower];
    start += 32;
  }
...
#pragma HLS UNROLL
    size_t end = start + 31;
    link_out[2].range(end, start) = stitchedTowerLevelECALSummary[tower];
    start += 32;
  }

  link_out[3] = 0;
  link_out[3].range(31, 0) = ecalSummary;
  link_out[3].range(383, 32) = link_out[2].range(383, 32);
